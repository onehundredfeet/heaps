...
class MemoryManager {
	...
	public static inline final NATIVE_STRIDE = 4;

	@:allow(h3d)
	var driver:Driver;
	var floatBuffers:Array<ManagedBuffer>;
	var byteBuffers:Array<ManagedBuffer>;
	...
	inline function getBuffers(isBytes:Bool) {
		return isBytes ? byteBuffers : floatBuffers;
	}
	...
	**/
	...
	function allocManaged(m:ManagedBuffer) {
		...
		var mem = m.size * m.stride * NATIVE_STRIDE;
	...
	function freeManaged(m:ManagedBuffer) {
		...
		m.vbuf = null;
		usedMemory -= m.size * m.stride * NATIVE_STRIDE;
		bufferCount--;
		if (!m.flags.has(Managed)) {
			var buffers = getBuffers(m.flags.has(ByteBuffer));
	...
	function allocBuffer(b:Buffer, stride:Int) {
		...
		if (!b.flags.has(Managed)) {
			...
			if (b.flags.has(UniformBuffer)) {
				if (flags == null)
					flags = [];
				flags.push(UniformBuffer);
			}
			if (b.flags.has(ByteBuffer)) {
				if (flags == null)
					flags = [];
				flags.push(ByteBuffer);
			}
			//trace('FILTER Managed B Stride ${stride} Verts ${b.vertices} byteBuffer${flags != null ? flags.contains(ByteBuffer) : false}');
			var m = new ManagedBuffer(stride, b.vertices, flags);
			var buffers = getBuffers(b.flags.has(ByteBuffer));
			m.next = buffers[0];
			buffers[0] = m;
			if (!m.allocBuffer(b))
				throw "assert";
		...
		//trace('Managed A ${stride} Size ${SIZE}');
		m = new ManagedBuffer(stride, SIZE, b.flags.has(ByteBuffer) ? [Managed, ByteBuffer]: [Managed]);
		if (prev == null)
			buffers[stride] = m;
		else
			prev.next = m;
	...
	function allocDepth(b:h3d.mat.DepthBuffer) {
		...
		depths.push(b);
		texMemory += b.width * b.height * NATIVE_STRIDE;
	}
	...
	function freeMemorySize() {
		...
		for (i in 0...2) {
			var byteBuffer = i == 0;
			var buffers = getBuffers(byteBuffer);
			for (b in buffers) {
				var b = b;
				while (b != null) {
					var free = b.freeList;
					while (free != null) {
						size += free.count * b.stride * (byteBuffer ? 1 : NATIVE_STRIDE);
						free = free.next;
					}
					b = b.next;
				}
	...
	public function stats() {
		...
		for (i in 0...2) {
			var byteBuffers = i == 0;
			var buffers = getBuffers(byteBuffers);
			var stride_size = byteBuffers ? 1 : NATIVE_STRIDE;

			for (b in buffers) {
				var b = b;
				while (b != null) {
					total += b.stride * b.size * stride_size;
					var f = b.freeList;
					while (f != null) {
						free += f.count * b.stride * stride_size;
						f = f.next;
					}
					count++;
					b = b.next;
	 ...
	 */
	@:access(h3d.Buffer)
	public function allocStats():Array<{
		position:String,
		count:Int,
		tex:Bool,
		size:Int,
		stacks:Array<{stack:String, count:Int, size:Int}>
	}> {
		#if !track_alloc
		return [];
		#else
		...
		for (buf in buffers) {
			...
			while (buf != null) {
				...
				while (b != null) {
					...
					inf.count++;
					var size = b.vertices * b.buffer.stride * NATIVE_STRIDE;
					inf.size += size;
