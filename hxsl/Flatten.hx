...
class Flatten {
	...
	public function flatten(s:ShaderData, kind:FunctionKind, constsToGlobal:Bool):ShaderData {
		...
		var textures = packTextures(prefix + "Textures2D", allVars,
			...
			.concat(packTextures(prefix + "TexturesRT", allVars, TChannel(1)));
	...
	function mapExpr(e:TExpr):TExpr {
		e = switch (e.e) {
			case TVar(v):
				var a = varMap.get(v);
				if (a == null) e else access(a, v.type, e.p, AIndex(a));
						...
						case TArray(t, _):
							...
							if (a.v.hasQualifier(Distinct)) {
								...
								if (stride == 0 || stride & 3 != 0)
									throw new Error("Dynamic access to an Array which size is not 4 components-aligned is not allowed", e.p);
								stride >>= 2;
							}
							eindex = toInt(mapExpr(eindex));
	...
	function mapConsts(e:TExpr):TExpr {
		switch (e.e) {
			...
			case TCall({e: TGlobal(Vec4)}, [
				{e: TVar({kind: Global | Param | Input | Var}), t: TVec(3, VFloat)},
				{e: TConst(CInt(1))}
			]):
	...
	function access(a:Alloc, t:Type, pos:Position, acc:ARead):TExpr {
		...
		switch (t) {
			case TMat4:
				return {
					e: TCall({e: TGlobal(Mat4), t: TFun([]), p: pos}, [read(0, pos), read(1, pos), read(2, pos), read(3, pos),]),
					t: TMat4,
					p: pos
				}
			case TMat3x4:
				...
				return {
					e: TCall({e: TGlobal(Mat3x4), t: TFun([]), p: pos}, [read(0, pos), read(1, pos), read(2, pos),]),
					t: TMat3x4,
					p: pos
				}
			case TMat3:
			...
			case TArray(t, SConst(len)):
				...
				var earr = [
					for (i in 0...len) {
						var a = new Alloc(a.g, a.t, a.pos + stride * i, stride);
						access(a, t, pos, AIndex(a));
					}
				];
	...
	function packTextures(name:String, vars:Array<TVar>, t:Type) {
		...
		for (v in vars) {
			...
			if (v.type != t) {
				switch (v.type) {
					case TChannel(_) : if (!t.match(TChannel(_))) continue;
	...
	function gatherVar(v:TVar) {
		switch (v.type) {
			...
			default:
				switch (v.kind) {
					case Global:
						if (v.hasQualifier(PerObject)) params.push(v); else globals.push(v);
					case Param:
						params.push(v);
					default:
						outVars.push(v);
